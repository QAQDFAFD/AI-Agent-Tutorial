...
| [14](docs/14-teaching-agent.md) | 最终实战：生产级教学 Agent 站点 | [`tutor/`](tutor/) | 阅读器否；聊天需要 |
...
启动最终实战案例——教程站点 + 教学助手（[第 14 章](docs/14-teaching-agent.md)）：

```bash
uv run uvicorn tutor.api.app:app --reload
# 打开 http://127.0.0.1:8000；不配置 Key 时阅读器可用、聊天优雅降级
```
...
```text
.
├── docs/                 # 15 个循序渐进的中文章节 + 环境准备/术语表附录
├── demos/                # 与章节一一对应的完整 Python Demo
├── tutor/                # 最终实战：生产级教学 Agent 站点（第 14 章）
